  ...
  def authenticator
    Rack::OAuth2::Server::Token.new do |req, res|
      ...
      when :authorization_code
        ...
        res.access_token = code.access_token.to_mac_token(:with_refresh_token)
      when :password
        ...
        res.access_token = account.access_tokens.create(:client => client).to_mac_token(:with_refresh_token)
      when :client_credentials
        # NOTE: client is already authenticated here.
        res.access_token = client.access_tokens.create.to_mac_token
      when :refresh_token
        refresh_token = client.refresh_tokens.valid.find_by_token(req.refresh_token)
        req.invalid_grant! unless refresh_token
        res.access_token = refresh_token.access_tokens.create.to_mac_token
      else
